...
def gen_split(root_dir, stackSize, phase):
    ...
    for dir_user in sorted(os.listdir(root_dir)):
        print(os.path.basename(dir_user))
        if os.path.basename(dir_user) != 'S2' and phase == 'train':
            class_id = 0
                ...
                if insts != []:
                    for inst in insts:
                        ...
                        if numFrames >= stackSize:
                            ...
                            #print(DatasetY)
                class_id += 1
            print(str(len(DatasetX)) + " " + str(class_id-1))
        elif os.path.basename(dir_user) == 'S2' and phase != 'train':
            class_id = 0
            dir = os.path.join(root_dir, dir_user)
            #print(dir)
            for target in sorted(os.listdir(dir)):
